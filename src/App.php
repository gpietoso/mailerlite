...
use \MailerLite\FieldModel as FieldModel;
use \MailerLite\SubscriberModel as SubscriberModel;
            ...
            $this->map(['POST', 'PUT', 'PATCH'], '', function (Request $request, Response $response, $args) {
                ...
                if (!array_key_exists('payload', $requestBody)) {
                    ...
                    if (!Validation::meetsMinimumFieldRequirement($parsedData->fields)) {
                        return $response->withJson(['message' => "Payload does not contain minium 
															subscriber fields (name, email_address, state)"], 503);
                    }

                    // Validates if email is valid (formatting and active dns)
                    if (!Validation::isValidEmail($parsedData->fields)) {
                        return $response->withJson(['message' => "Invalid e-mail address format 
																	or domain is not active"], 503);
                    }
                ...
                if (!array_key_exists('payload', $requestBody)) {
                    ...
                    if (!Validation::isValidType($parsedData->type)) {
                        ...
                        if ($return) {
                            ...
                            return $response->withJson(['message' => "Error"], 404);
                        }
                    }
                }
